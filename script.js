...
function chooseSpread(cards,key){
...
function shuffleDeck(){
    ...
    moves.forEach(move=>{
        ...
        div.innerHTML = `<img src="cards/card_back.jpg">`;
...
function pickCard(index,element){
    ...
    card.reversed = dailyMode ? false : Math.random() < 0.5;
...
function updateSelectedText(){
    ...
    if(selectedCount){
        ...
        `已選 ${selectedCards.length} / ${spreadType} 張`;
...
function copyReadingContent(){
    ...
    shareCard.classList.add("single-download");

}else{

    shareCard.classList.remove("single-download");
...
function downloadShareImage(){
    ...
    const shareQuestion =
    document.getElementById("shareQuestion");

    const shareCards =
    document.getElementById("shareCards");

    const question =
    document.getElementById("userQuestion").value || "塔羅占卜";
...
function showResultGuide(){
    ...
    let guide =
    document.getElementById("resultGuide");
    ...
    guide.style.padding = "8px 14px";
    guide.style.borderRadius = "999px";

    guide.style.background = "rgba(0,0,0,.48)";
